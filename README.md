...
`config.yaml` besitzt geschlossene Modelle für IMAP, Telegram, Ziele, Limits, Wiederholungen, Timeouts und Logging. Die LLM-Wiederholungen für ungültige Providerantworten, JSON-Reparatur und Schema-Reparatur sind getrennt begrenzt. IMAP, Telegram, OpenRouter und Todoist haben jeweils eigene Werte für Timeout, Retry-Anzahl sowie initialen und maximalen Backoff. Validiert werden insbesondere Port, Polling, Adaptertimeouts, Mailgröße, LLM-Rate, Wiederholungszahlen, IANA-Zeitzone, eindeutige nichtleere Ordner, sichere Pfade und die Log-Level `DEBUG`, `INFO`, `WARNING`, `ERROR`, `CRITICAL`. Unbekannte Schlüssel und falsche Typen werden abgelehnt.
...
Mailzustands-Schema 7 migriert Schema 6 ausdrücklich, indem der früher gemeinsame
Benachrichtigungsstatus konservativ auf Summary- und Vorschlagsversand abgebildet
und die Datei sofort atomar als Schema 7 gespeichert wird. Schema 7 bleibt gegenüber
...
3. Den Checkpoint für genau dieses Konto und diesen Ordner bei gestopptem Dienst
   ...
   `.invalid`-Datei als Nachweis gesichert lassen und nicht in Schema 7
   umetikettieren oder manuell mit erfundenen Pflichtfeldern ergänzen.
4. Mailhelp mit der aktuellen Version starten, die Mail neu als Schema 7 einlesen
