...
from ._core import (
    ActionLedgerPort, Decision, DecisionAction, ProposalPersistence,
    TelegramTransport, WriteExecution, apply_decision,
)
...
class ProposalDecisionService:
    ...
    def __init__(self, store: JsonStore, repository: ProposalRepository,
                 persistence: ProposalPersistence, ledger: ActionLedgerPort,
                 writes: WriteExecution, telegram: TelegramTransport,
                 allowed_user: int, allowed_chat: int):
    ...
    def decide(self, decision: Decision, user_id: int, chat_id: int) -> str:
        ...
        proposal = self.repository.load_current(
            decision.mail_id, decision.proposal_id)
        if proposal is None:
            raise ValueError("Vorschlag wurde nicht gefunden")
        if ((proposal.source_mail_id, proposal.id, proposal.version) !=
                (decision.mail_id, decision.proposal_id, decision.version) or
        ...
        if decision.action == DecisionAction.EDIT:
            ...
            self.store.save("telegram-dialog", TelegramDialogState(
                mail_id=proposal.source_mail_id, proposal_id=proposal.id,
                version=proposal.version).model_dump())
            prompt = (proposal.open_questions[0] if proposal.open_questions
        ...
        if (decision.action in {
                DecisionAction.CONFIRM, DecisionAction.CONFIRM_DUPLICATE} and
        ...
        confirming = decision.action in {
            DecisionAction.CONFIRM, DecisionAction.CONFIRM_DUPLICATE}
        ...
        if decision.action == DecisionAction.CONFIRM and duplicates:
            ...
            self.telegram.send(self.allowed_chat, "\n".join([
                ...
                {"text": "Erneut anlegen", "callback_data": duplicate.encode(self.store)},
                {"text": "Nicht erneut", "callback_data": reject.encode(self.store)},
            ]]})
        ...
        changed = (proposal.model_copy(update={"status": ProposalStatus.REJECTED})
                   ...
                   apply_decision(proposal, decision.model_copy(
                       update={"action": DecisionAction.CONFIRM}),
                       user_id, chat_id, self.allowed_user, self.allowed_chat))
        self.persistence.persist(changed)
