...
from .config import Topic, TopicsConfig
...
def save_topics(path: Path, existing: list[Topic], accepted: list[LearnedCategory]) -> list[Topic]:
    """Validate and atomically replace topics.yaml with accepted candidates."""
    ...
    result = TopicsConfig(topics=[*existing, *additions]).topics
...
class LearningMode:
    ...
    def run(self, count: int) -> int:
        ...
        if not classifications:
            self.output("Keine Mails zum Lernen gefunden; topics.yaml wurde nicht geändert.")
            return 0
        ...
        self.output(f"{len(accepted)} neue Kategorien wurden gespeichert.")
        return len(accepted)
