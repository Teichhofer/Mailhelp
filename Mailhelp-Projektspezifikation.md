...
- Die Abrufstrategie ist fest **„neueste zuerst mit dauerhaftem Backlog“**. Innerhalb jedes Ordners werden noch nicht dauerhaft übernommene UIDs absteigend verarbeitet; während des Backlog-Abbaus eingetroffene höhere UIDs haben im nächsten Poll Vorrang. `batch_size` begrenzt den Abruf pro Ordner und Polling-Zyklus auf `1..1000` Nachrichten (Standard `25`). Bei einem begrenzten Einmallauf reduziert das nach Wiederaufnahmen verbleibende `--max-mails`-Budget bereits die Zahl vollständig abgerufener Nachrichten. Nach der Suche werden die noch offenen und ausgewählten Anzahlen sowie nach jedem schreibfreien Nachrichtenabruf der Fortschritt ohne Mailinhalt protokolliert.
- Ein konto-, UIDVALIDITY- und ordnerbezogener Checkpoint speichert die einmalig bestimmte untere `start_uid`, eine nur informative höchste übernommene UID und normalisierte, disjunkte Bereiche aller dauerhaft übernommenen UIDs. Die höchste UID allein darf niemals niedrigere Lücken erledigen. Nach jedem Verarbeitungsergebnis mit garantiert gespeichertem Mailzustand wird genau dessen UID atomar dem Bereichszustand hinzugefügt; ein unerwarteter Abbruch ohne solchen Zustand lässt diese UID offen. Dadurch werden erfolgreich beziehungsweise dauerhaft als wartend/fehlgeschlagen übernommene UIDs nach Neustarts nicht erneut regulär verarbeitet, während offene Lücken, mehrere Batches und neue höhere UIDs erhalten bleiben. `_resume_pending` bleibt unabhängig davon für fällige dauerhafte Mailzustände zuständig und belastet bei `--max-mails` dasselbe Verarbeitungskontingent.
- Bei einem UIDVALIDITY-Wechsel werden sämtliche UID-Bereiche des betroffenen Ordners verworfen. Ohne `historical_start` beginnt die neue UID-Namenswelt bei `0`; mit konfigurierter historischer Grenze wird deren absoluter Zeitpunkt gegen die neue UID-Namenswelt erneut aufgelöst, bevor eine Mail daraus verarbeitet wird. Andere Ordner und Konten bleiben unberührt.
