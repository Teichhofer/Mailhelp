...
- `global_newest_first` ist ein boolescher Schalter. Bei `true` werden zunächst die UID- und `INTERNALDATE`-Metadaten aller noch offenen Nachrichten aus allen konfigurierten Ordnern schreibfrei ermittelt und als gemeinsame, nach Empfangszeitpunkt absteigend sortierte Warteschlange behandelt. Erst danach werden die durch das gemeinsame `batch_size`-, `--max-mails`- oder `--learn`-Kontingent ausgewählten Nachrichten mit `BODY.PEEK[]` geladen. Die ordnerspezifischen Checkpoints und der dauerhafte Backlog bleiben erhalten. Bei `false` werden die Ordner weiterhin in Konfigurationsreihenfolge und jeweils neueste UID zuerst verarbeitet.
