...
- `batch_size` begrenzt den Abruf pro Polling-Zyklus auf `1..1000` Nachrichten (Standard `25`). Nach der Suche werden die gefundene und ausgewählte Anzahl sowie nach jedem schreibfreien Nachrichtenabruf der Fortschritt ohne Mailinhalt protokolliert; der persistierte UID-Checkpoint setzt den nächsten Zyklus fort.
...
Ein ausdrücklich in der Mail genannter physischer Ort wird getrennt von einem Videolink in `location` beziehungsweise `video_link` übernommen; fehlende Werte bleiben `null` und dürfen nicht erfunden werden. `video_link` akzeptiert ausschließlich längenbegrenzte HTTP-/HTTPS-URLs. Vor einer Bestätigung zeigt Telegram beide Felder sichtbar an. Beim Schreiben nach Google Calendar wird `location` auf das gleichnamige Ereignisfeld abgebildet. Ein vorhandener Videolink wird am Ende von `description` in einem klar markierten Abschnitt `[Mailhelp-Videolink]` abgelegt. Diese Abbildung erzeugt keine Google-Meet-Konferenz und überträgt kein `conferenceData`; eine Konferenzerzeugung müsste separat implementiert und bestätigt werden.
...
| `.env` | IMAP-Zugangsdaten, OpenRouter-Key, Telegram-Bot-Token sowie Google- und Todoist-OAuth-Zugangsdaten |
...
Todoist-Client-ID, -Client-Schlüssel und der nach einmaliger Nutzerautorisierung
ausgegebene Refresh-Token werden als `TODOIST_CLIENT_ID`,
`TODOIST_CLIENT_SECRET` und `TODOIST_REFRESH_TOKEN` ausschließlich aus `.env`
oder der Prozessumgebung geladen. Mailhelp tauscht sie im Arbeitsspeicher gegen
kurzlebige Bearer-Tokens aus; ein persönlicher Todoist-API-Schlüssel wird nicht
verwendet. Alle drei Werte werden als Geheimnisse behandelt und bei fehlender
oder leerer Angabe bereits beim Start abgelehnt.
