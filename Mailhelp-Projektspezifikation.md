...
- `historical_start` ist optional (`null`) oder ein zeitzonenbehafteter ISO-8601-Zeitpunkt mit explizitem Offset. Seine absolute UTC-Grenze wird über schreibfreie `UID SEARCH`-/`UID FETCH INTERNALDATE`-Abfragen sekundengenau aufgelöst. Die Auswertung akzeptiert direkte Bytes-Metadaten und das erste Bytes-Element eines Antwort-Tupels, ignoriert strukturelle Abschlussfragmente und erkennt `INTERNALDATE` ohne Beachtung der Groß-/Kleinschreibung. Der resultierende Start-UID wird vor der Verarbeitung konto- und ordnerbezogen gespeichert und nach Neustarts nicht neu interpretiert.
